...
## Post-Sprint-2 fixes — site-wide typography

### 26. Mono typeface swapped from Space Mono to Courier Prime (approved departure from the golden master's typography)

- **Date:** 2026-08-26.
- **Route / component:** site-wide — every element styled with
  `var(--font-mono)` (folio/kicker lines, the "ISSUE 05 · AUG 2026" folio
  bar, the "OPEN TO WORK" badge, badges/status labels, the Dispatch band,
  the publication footer, and every other mono-styled label across every
  page). `src/styles/fonts.css`, `astro.config.mjs`,
  `src/components/FontFaces.astro`.
- **Immutable source behavior/design:** The golden master's own Google
  Fonts request specifies Space Mono for every one of these elements
  (`Newsstand - Full Site.dc.html`, throughout — every `'Space Mono'`
  font-family declaration).
- **Production behavior/design:** `--font-mono` now resolves to Courier
  Prime instead. Every element that reads `var(--font-mono)` picks this up
  automatically — no per-page or per-component changes were needed, since
  the whole codebase already routed mono typography through this one
  token rather than naming Space Mono directly. Weights 400/700 are both
  real Courier Prime faces (not browser-synthesized fake bold), so every
  existing `font-weight: 700` mono label sitewide renders a true bold
  glyph exactly as before, just in the new face.
- **Reason for deviation:** The project owner explicitly requested this
  site-wide change directly, after Sprint 2 completed, referencing an
  actual Royal Quiet Deluxe typewriter as the target aesthetic and
  explicitly authorizing any typewriter-styled alternative. Courier Prime
  was chosen over more heavily distressed options (e.g. Special Elite)
  specifically because it offers genuine 400/700 weights, avoiding
  faux-bold rendering across the many mono labels that rely on
  font-weight 700. This is a deliberate, requested departure from the
  golden master's own typography, not a fidelity bug — AGENTS.md's
  "preserve the experience" mandate governs reproducing the golden
  master's design, not a typography change the actual project owner asks
  for on top of it (same reasoning as entry 25's dot pulse).
- **Classification:** approved enhancement, not a golden-master deviation
  in the usual "we had to compromise" sense.
- **Status:** implemented, pending the project owner's visual confirmation
  across all pages/viewports.
- **Approval source:** project owner, reported directly post-Sprint-2.
- **Exact files/nodes affected:** `astro.config.mjs` (the font provider
  entry renamed from `Space Mono`/`--font-space-mono` to `Courier
Prime`/`--font-courier-prime`); `src/styles/fonts.css` (`--font-mono`
  repointed); `src/components/FontFaces.astro` (the preloaded `<Font>`
  cssVariable updated to match). Every page's own file-level comments that
  quote the golden master's literal `'Space Mono'` style declarations as
  historical citations of the immutable source are left untouched — those
  accurately describe what the reference file itself specifies, not the
  live rendered font.
- **Testing:** `tests/visual.spec.ts` baselines were regenerated for every
  route at all three viewports and inspected directly (font renders
  correctly, real bold weight, no text overflow/wrapping regressions from
  Courier Prime's different character metrics vs. Space Mono).
  `tests/smoke.spec.ts`/`tests/accessibility.spec.ts` continue to pass
  unchanged, since none of them assert a specific font-family value.
- **Final-QA reminder:** if the project owner prefers a different
  typewriter-styled alternative after seeing this live, swap the single
  `astro.config.mjs` font entry and `fonts.css`'s `--font-mono` line —
